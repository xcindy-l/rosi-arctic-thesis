...
f_out = os.path.join(output_dir, f"{filename}_snow_1deg.nc")

# Open dataset
ds = xr.open_dataset(f_in)

# Check for 'vsnon_d'
if 'vsnon_d' not in ds.data_vars:
    print(f"Skipping {filename}: no vsnon_d variable.")
    sys.exit(0)
...
ds_temp = ds[['vsnon_d']].rename({'TLON': 'lon', 'TLAT': 'lat'})
...
for year in np.unique(ds['time'].dt.year):
    ...
    ds_temp_year = ds_year[['vsnon_d']].rename({'TLON': 'lon', 'TLAT': 'lat'})
    
    ds_regridded = regridder(ds_temp_year['vsnon_d'])
    ds_regridded.name = 'vsnon_d'
