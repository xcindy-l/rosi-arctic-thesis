...
INPUT_DIR="/glade/campaign/cgd/cesm/CESM2-LE/ice/proc/tseries/day_1/aice_d"
OUTPUT_DIR="/glade/work/xliu1/cesm2_seaice_regrid"
mkdir -p "$OUTPUT_DIR"
...
for f in "$INPUT_DIR"/*smbb*.nc; do
    ...
    output_file="$OUTPUT_DIR/${filename}_aice_1deg.nc"
    ...
    "$PYTHON_BIN" "$OUTPUT_DIR/cesm2_seaice_regrid.py" "$f" "$OUTPUT_DIR" "$filename"
done
